...
def placebid(request, listing_id):
    ...
    if request.method == "POST":
        ...
        allbids = len(bids)
        lastbid = Bid.objects.filter(listing=listing_id).last()
...
def mylisting(request):
    currentUser = request.user
    listings = Listing.objects.all().filter(owner=currentUser)
...
def mybids(request):
    currentUser = request.user
    bids = Bid.objects.all().filter(bidder=currentUser).order_by("-created_at")
    lastbid = Bid.objects.last()
